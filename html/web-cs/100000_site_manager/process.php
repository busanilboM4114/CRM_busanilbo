...
{
	$error_link='?menu='.$menu.'&tn='.$tn.'&mode=modify&idx='.$idx.'&search_select='.$search_select.'&keyword='.@htmlspecialchars($keyword).'&pagenum='.$pagenum;//실패 후 이동 경로
	$success_link='?menu='.$menu.'&tn='.$tn.'&mode=modify&idx='.$idx.'&search_select='.$search_select.'&keyword='.@htmlspecialchars($keyword).'&pagenum='.$pagenum;//성공 후 이동 경로
}
...
else if($modes=='menu_write' || $modes=='menu_modify')
{
	$error_link='?menu='.$menu.'&tn='.$tn.'&mode=menu&site_idx='.$site_idx.'&search_select='.$search_select.'&keyword='.@htmlspecialchars($keyword).'&pagenum='.$pagenum;//실패 후 이동 경로
	$success_link='?menu='.$menu.'&tn='.$tn.'&mode=menu&site_idx='.$site_idx.'&search_select='.$search_select.'&keyword='.@htmlspecialchars($keyword).'&pagenum='.$pagenum;//성공 후 이동 경로
}
else
{
...
switch($modes)
{
	case 'write':
		//필수값 체크
		if($_POST['mode']=='' || $_POST['modes']=='' || $_POST['title']=='')
		{
			script_alert_replace('필수 입력값이 없습니다.\\n에러코드[w001]', $error_link);
			exit;
		}
		...
		break;
	case 'modify':
		if($_POST['mode']=='' || $_POST['modes']=='' || $_POST['idx']=='')
		{
			script_alert_replace('필수 입력값이 없습니다.\\n에러코드[m001]', $error_link);
			exit;
		}
	...
	case 'menu_write':
		//필수값 체크
		if($_POST['mode']=='' || $_POST['modes']=='' || $_POST['site_idx']=='')
		{
			script_alert_replace('필수 입력값이 없습니다.\\n에러코드[mw001]', $error_link);
			exit;
		}

		extract($_POST);

		$sql="select * from `site_menu` where `idx`='-1'";
		@$rs_sql=$conn->Execute($sql);

		if(!$rs_sql)
		{
			script_alert_replace('DB 질의가 실패했습니다.\\n에러코드[mw005]', $error_link);
			$rs_sql->close();
			exit;
		}
		...
		@$rs_max_no=$conn->Execute($sql);

		$sort=$rs_max_no->fields['max_no']=='' ? 1 : $rs_max_no->fields['max_no']+10;
	...
	case 'menu_modify':
		...
		{
			$title				= $title_[$i];
			$menu_link			= $menu_link_[$i];
			$new_window_flag	= ${'new_window_flag_'.$idx[$i]};
			$onoff				= ${'onoff_'.$idx[$i]};
			$sort				= $sort_[$i];

			$sql="select * from `site_menu` where `idx`='".$idx[$i]."'";
			@$rs_sql=$conn->Execute($sql);

			if(!$rs_sql)
			{
				script_alert_replace('DB 질의가 실패했습니다.\\n에러코드[mm005]', $error_link);
				$rs_sql->close();
				exit;
			}
			...
			if(!$rs_desc)
			{
				script_alert_replace('DB 질의가 실패했습니다.\\n에러코드[mm010]', $error_link);
				$rs_sql->close();
				exit;
			}

			//제외되어야 될 필드
			$val_esc=array('user_idx', 'user_id', 'user_name', 'idx');
