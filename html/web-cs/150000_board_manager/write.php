...
else//쓰기가 아닐경우 수정, 쓰기/수정 이외의 것은 process에서....
{
	...
	if($rs_board->_numOfRows)
	{
		foreach($rs_board->fields as $key => $value)
		{
			...
			if($key=='contents' && $value)
			{
				$value_decode=json_decode($value);
				for($i=0;$i<count($value_decode);$i++)
				{
					unset($values);
					if(is_array($value_decode[$i]->value))
					{
						for($j=0;$j<count($value_decode[$i]->value);$j++)
						{
							$values[]=$value_decode[$i]->value[$j];
						}
					}

					$add_fields[]=array(
							'name'=>$value_decode[$i]->name,
							'label'=>$value_decode[$i]->label,
							'type'=>$value_decode[$i]->type,
							'value'=>$values
						);
				}
		...
		{
			$fix_rows['addition_fields']=$add_fields;
			$fix_rows['addition_fields_cnt']=count($add_fields);
		}
	}
	else
	{
