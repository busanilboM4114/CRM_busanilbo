...
else if($modes=='email_send')
{
	$error_link='?menu='.$menu.'&tn='.$tn.'&mode=email_send&search_select='.$search_select.'&keyword='.@htmlspecialchars($keyword).'&pagenum='.$pagenum;//실패 후 이동 경로
	$success_link='?menu='.$menu.'&tn='.$tn.'&mode=email_send&search_select='.$search_select.'&keyword='.@htmlspecialchars($keyword).'&pagenum='.$pagenum;//성공 후 이동 경로
}
else if($modes=='sms_send')
{
	$error_link='?menu='.$menu.'&tn='.$tn.'&mode=sms_send&search_select='.$search_select.'&keyword='.@htmlspecialchars($keyword).'&pagenum='.$pagenum;//실패 후 이동 경로
	$success_link='?menu='.$menu.'&tn='.$tn.'&mode=sms_send&search_select='.$search_select.'&keyword='.@htmlspecialchars($keyword).'&pagenum='.$pagenum;//성공 후 이동 경로
}
else
{
...
switch($modes)
{
	...
	case 'alldel':
		...
		break;
	case 'email_send':
		if($_REQUEST['mode']=='' || $_REQUEST['modes']=='' || $_REQUEST['target']=='')
		{
			script_alert_replace('필수 입력값이 없습니다.\\n에러코드[es001]', $error_link);
			//print_r($_POST);
			exit;
		}
		...
		script_alert_replace('메일이 발송되었습니다.', $success_link);

		break;
	case 'sms_send':
		if($_REQUEST['mode']=='' || $_REQUEST['modes']=='' || $_REQUEST['target']=='')
		{
			script_alert_replace('필수 입력값이 없습니다.\\n에러코드[ss001]', $error_link);
			//print_r($_POST);
			exit;
		}
		...
		script_alert_replace("SMS가 발송되었습니다.", $success_link);
