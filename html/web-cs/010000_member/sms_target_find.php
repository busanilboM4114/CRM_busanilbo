...
while(!$rs_board->EOF)
{
	...
	foreach($rs_board->fields as $key => $value)
	{
		...
		if($key=='mb_hp')
		{
			$sms_send_hp=str_replace('-', '', $value);
		}
		$fix_rows[$rs_board->_currentRow][$key]=$value;
	}
	$fix_rows[$rs_board->_currentRow]['no']=$listnum--;

	$fix_rows[$rs_board->_currentRow]['sms_send_hp']=$sms_send_hp;

	if($fix_rows[$rs_board->_currentRow]['mb_leave_date'])
	{
		$leave_msg = '<span class="mb_leave_msg">탈퇴함</span>';
	}
	else if ($fix_rows[$rs_board->_currentRow]['mb_intercept_date'])
	{
	...
	{
		$fix_rows[$rs_board->_currentRow]['status_msg']=$leave_msg.' '.$intercept_msg;
	}
	else
	{
		$fix_rows[$rs_board->_currentRow]['status_msg']='정상';
	}

	switch($join_type_icon[0])
	{
		case 'naver':
			$fix_rows[$rs_board->_currentRow]['mb_icon']='<img src="./img/naver.svg" width="15" height="15" style="line-height: 15px;">';
			break;
		case 'kakao':
			$fix_rows[$rs_board->_currentRow]['mb_icon']='<img src="./img/kakao.svg" width="15" height="15" style="line-height: 15px;">';
			break;
		case '':
		default:
			$fix_rows[$rs_board->_currentRow]['mb_icon']='<img src="./img/busanilbo.svg" width="15" height="15" style="line-height: 15px;">';
			break;
	}
...
if($FILE_SKIN=='_a')
{
	$opt='&join_type='.$join_type.'&mode='.$mode;
...
##템플릿 페이지 연결#################
$main_contents = $FILE_SKIN."sms_target_find.tpl.htm";
